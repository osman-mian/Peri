...
class Orion:
	...
	def optimize(self):
		print("Running Orion++ (parallel, until convergence)");
		converge=False;
		iters=1;
		dims=self.V;
		while not converge and iters<=1000:
			gc.collect();
			print("Iter(",iters,"): Calculating edge addition scores")
			self.init_forward();
			print("Iter(",iters,"): Forward Search")
			converge= self.forward()
			...
			print("Iter(",iters,"): Calculating edge removal scores")
			self.init_backward(self.E);
			print("Iterer(",iters,"): Backward Search")
			converge= self.backward() and converge;	#order matters! in case there is short-circuit switching
			self.E=[];
			if not converge:
				for i in range(dims):
					for j in range(i+1,dims):
						if self.network[i,j]==0 and self.network[j,i]==0 and i!=j:
							self.E.append((i,j));
			iters=iters+1;
			print("Iter(",iters,"): Finished")
